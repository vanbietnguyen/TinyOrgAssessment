...
from base.serializers import RecipeSerializer, AllergenSerializer
from base.models import Recipe, Allergen
...
@api_view(['POST'])
def postAllergens(request):
    data = request.data
    try:
        allergen = Allergen.objects.create_user(
            name = data['name']
        )
    except:
